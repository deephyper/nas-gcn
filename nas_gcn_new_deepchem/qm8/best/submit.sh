...
srun -n 30 python -m tuster.system.bebop.run 'python -m deephyper.search.nas.regevo --evaluator ray --redis-address {redis_address} --problem nas_gcn.qm8.problem.Problem'

# Start cluster wit 30 nodes with random search
# srun -n 30 python -m tuster.system.bebop.run 'python -m deephyper.search.nas.full_random --evaluator ray --redis-address {redis_address} --problem nas_gcn.qm8.problem.Problem'